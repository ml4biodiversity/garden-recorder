...
def FrontierLabs_BLT_slice_to_fragments(conf):
    ...
    # Load meta data
    files = list(Path(fl_path).rglob("*.wav"))
...
def AudioMoth_slice_to_fragments(conf):
    fl_path = conf["audio_path"]
    data_name = conf["data_name"]  # Descriptive name this data set
    number_of_frames = conf["number_of_frames_per_fragment"]

    # Load meta data
    files = [str(x) for x in Path(fl_path).glob("*.WAV")]
    meta = pd.DataFrame({"Filename":files})
    meta["time"] = conf["start_time"]
    meta["time"] = pd.to_datetime(meta["time"], format="%d-%m-%Y %H:%M:%S")
    st0 = meta.loc[0,"time"]
    meta["time"] = [st0+dt.timedelta(hours=12*c1) for c1 in range(meta.shape[0])]
